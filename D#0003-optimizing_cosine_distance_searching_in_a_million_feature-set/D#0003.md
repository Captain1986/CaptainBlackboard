...
本文只是抛砖引玉，初步涉及了优化汪洋大海中的一点点。光优化的层次来分就有系统级的，应用级，代码级别的，优化还要区分是网络瓶颈，IO瓶颈，内存瓶颈还是CPU瓶颈，本文的这个小demo只涉及到代码级别的单个通用CPU和内存优化，还没有涉及到用上GPU(十分适合做大规模并行计算)，负责均衡和第三方线性代数计算库(例如Eigen，MKL，OpenBlas)。
...
|    Step 12(浮点转定点)     |         31298         |         ~x2.02         |     **x123.19**      | 内存效率 |
